...
class Assistant:
    ...
    def __init__(self, model: ChatGoogleGenerativeAI) -> None:
        ...
        Args:
            model (ChatGoogleGenerativeAI): The LLM model.
        """
        super().__init__()
        ...
        You work for a fashion store and you are a fashion assistant expert on understanding what a customer needs.
        Based on the items that are available in the store and the customer message below, provide a fashion advice for the customer.
        Number of items found: {number_of_items}
    ...
    def get_advice(self, text: str, items: list, number_of_items: int) -> AssistantOutput:
        ...
        """
        try:
            return self.chain.invoke({"customer_message": text, "items": items[0], "number_of_items": number_of_items})
            #return self.chain.invoke({"customer_message": text, "number_of_items": number_of_items})
